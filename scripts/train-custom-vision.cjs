...
function apiRequest(method, urlStr, headers, body = null) {
  return new Promise((resolve, reject) => {
    ...
    // Auto-calculate Content-Length to avoid chunked transfer issues on Azure
    const finalHeaders = { ...headers };
    if (body) {
      finalHeaders['Content-Length'] = Buffer.isBuffer(body) ? body.length : Buffer.byteLength(body);
    }
...
async function run() {
  ...
  try {
    // 0. Cleanup old projects to avoid Free tier limit (max 2 projects)
    console.log('0. Checking for old projects to clean up...');
    const listUrl = `${TRAINING_ENDPOINT}customvision/v3.0/training/projects`;
    try {
      const projects = await apiRequest('GET', listUrl, headers);
      if (projects && projects.length > 0) {
        for (const p of projects) {
          if (p.name === PROJECT_NAME) {
            console.log(`   Deleting old project: ${p.name} (${p.id})...`);
            const deleteUrl = `${TRAINING_ENDPOINT}customvision/v3.0/training/projects/${p.id}`;
            await apiRequest('DELETE', deleteUrl, headers);
            console.log('   Deleted successfully.');
          }
        }
      }
    } catch (err) {
      console.log('   (Skipped cleanup or no projects found)');
    }
    ...
    for (const cls of classes) {
      ...
      for (let i = 0; i < IMAGES_PER_CLASS; i++) {
        ...
        const uploadRes = await apiRequest('POST', uploadUrl, uploadHeaders, buffer);
        
        if (i === 0) {
          console.log(`      Sample response for first upload:`, JSON.stringify(uploadRes));
        }
      }
      console.log(`   ✅ Uploaded ${IMAGES_PER_CLASS} images for '${cls.name}'`);
    }

    // 4. Trigger Training
    console.log('4. Submitting model training job...');
    const trainUrl = `${TRAINING_ENDPOINT}customvision/v3.0/training/projects/${projectId}/train?trainingType=Quick`;
    ...
    const publishUrl = `${TRAINING_ENDPOINT}customvision/v3.0/training/projects/${projectId}/iterations/${iterationId}/publish?publishName=${PUBLISH_NAME}&predictionId=${encodeURIComponent(PREDICTION_RESOURCE_ID)}`;
